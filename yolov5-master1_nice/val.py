...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes, img_mask) in enumerate(tqdm(dataloader, desc=s)):
        ...
        # Run model
        # print("au___________________", augment)
        # out, train_out = model(img, augment=augment)  # inference and training outputs
        (out, train_out), mask_res = model(img, augment=augment, get_mask_res=True)  # inference and training outputs
        # print("out########################", out.shape)
        # print("########################", len(train_out))
        ...
        if compute_loss:
            ...
            loss += compute_loss([x.float() for x in train_out], targets, mask_res=mask_res, img_masks=img_mask)[
                1]  # box, obj, cls, mask_loss
        ...
        if plots and batch_i < 3:
            ...
            #print("%%%%%%label",targets)
            Thread(target=plot_images, args=(imgs_plot_true, targets, paths, f, names), daemon=True).start()

            imgs_plot_true = img.clone()
            for i, img_plot_true in enumerate(imgs_plot_true):
                img_mask_part = mask_res[i] > 0.5
                img_mask_part = img_mask_part.squeeze(0)
                img_plot_true[0, :, :][img_mask_part] = 1
                img_plot_true[1, :, :][img_mask_part] = 1
                img_plot_true[2, :, :][img_mask_part] = 1
            ...
            for i, img_plot_true in enumerate(imgs_plot_true):
                ...
                imgs_plot_true[i] = img_plot_true
            f = save_dir / f'val_batch{batch_i}_pred_0.55.jpg'  # predictions
            Thread(target=plot_images, args=(imgs_plot_true, output_to_target(out), paths, f, names),
                   daemon=True).start()
            ...
            for i, img_plot_true in enumerate(imgs_plot_true):
                ...
                imgs_plot_true[i] = img_plot_true
            f = save_dir / f'val_batch{batch_i}_pred_0.6.jpg'  # predictions
            Thread(target=plot_images, args=(imgs_plot_true, output_to_target(out), paths, f, names),
                   daemon=True).start()
