        ...
        if mask_res is not None:
            ...
            for i in range(len(paths)):
                ...
                if os.path.exists(name):
                    ...
                    mask_img = self.transf(mask_img).to(device=device).mean(dim=0, keepdim=True) / 255.
                    ...
                    mask_img = np.random.randint(10, size=(h_mask_img, w_mask_img, c_mask_img)).astype("float64")*0.0
                ...
                #mask_img = self.transf(mask_img).to(device=device).mean(dim=0, keepdim=True)/255.
                #mask_img[mask_img>0.2]=1

                #mask = (mask_res[i][0,:,:]).sigmoid()>(mask_res[i][1,:,:]).sigmoid()
                #mask = torch.where(mask==True,0,1).float().unsqueeze(0).unsqueeze(0)
