...
path_sma_mask = "D:\paCong/rope_pet_labeld\label_mask\mask/"

save_path = "D:/paCong/big_small_val/"
save_path_mask = save_path+"mask_big/"
max_num_small = 8

mask_rcnn_yaml_path = save_path+"yaml/"
mask_rcnn_yaml = False
...
if mask_rcnn_yaml and (not os.path.exists(mask_rcnn_yaml_path)):
    os.mkdir(mask_rcnn_yaml_path)
...
for index_big_file_name in range(len_data):#len(fileNames_big)
    ...
    for num in range(num_small):
        ...
        if fileNames_sma[ind_small_photo].find(".jpg")==-1:
            continue
        #print(path_sma + fileNames_sma[ind_small_photo])
        ...
        if os.path.exists(path_sma_mask + fileNames_sma[ind_small_photo].replace(".jpg", ".png")):
            ...
            # print(mask_sma.shape,red_change_while[:,:,0].shape, mask_sma[red_change_while[:,:,0],:].shape)
            # print("\n bejin"+np.where(mask_sma > 0))
            # print((mask_sma>0).shape)
            # print(mask_sma.shape, np.array([red_change_while[0]]*3).shape)
            # mask_sma[red_change_while[:,:,0],:]


            # mask_sma[[red_change_while[0]]*3] = 255
            # mask_sma[[red_change_while[1]]*3] = 255
            # mask_sma[[red_change_while[2]]*3] = 255
            # print("\n bejin" + np.where(mask_sma > 0))
    ...
    if val:
        ...
        cv2.imwrite(save_path_mask + fileNames_big[index_big_file_name], mask_big)
