...
firstDir = 'D:/paCong/background_new' # 总路径
...
def screen(url, select):
    ...
    # html = s.get(url=url, headers=headers)  # 你需要的网址
    # # html = requests.get(url=url, headers=headers)
    # html.encoding = 'gbk'  # 网站的编码
    # html = html.text
    # soup = BeautifulSoup(html, 'lxml')
...
# 定位到 1920 1080 分辨率图片               
def handleImgs(links, path, Pagenum):
    index_photo = 0
    for link in links:
        href = link.get('href')
        if(href == 'http://pic.netbian.com/' or href == 'https://pic.netbian.com/'): # 过滤图片广告
            continue
        ...
        if(requests.get(src).status_code == 404):
            ...
            continue
        print()
        download(src, str(Pagenum) + "-" + str(index_photo), "D:/paCong/background_new/风景/1")
        index_photo += 1
        time.sleep(interval)
...
def select_classification(choice):
    ...
    for i in range(108, lastPagenum):
        if i == 0:
            url = secondUrl
        else:
